export default {
	state: {
		...
		fileImg: localStorage.getItem("goudong_fileImg"),
		
		// 表格中显示的列
		selectedColumnList: localStorage.getItem('goudong_selected_column_list'),

		// 文件展示模式 0 列表模式 | 1 网格模式 | 2 时间线模式
		fileModel: localStorage.getItem('goudong_file_model'),

		// 网格模式 & 时间线模式下 图标大小 单位px
		gridSize: localStorage.getItem('goudong_grid_size')
			? Number(localStorage.getItem('goudong_grid_size'))
			: 80,
	...
	mutations: {
		...
		fileImg(state, imageUrl){
			localStorage.setItem("goudong_fileImg",imageUrl)
			state.fileImg = imageUrl
		},
		...
		changeSelectedColumnList(state, data) {
			localStorage.setItem('goudong_selected_column_list', data.toString())
		...
		changeFileModel(state, data) {
			localStorage.setItem('goudong_file_model', data)
		...
		changeGridSize(state, data) {
			localStorage.setItem('goudong_grid_size', data)
			state.gridSize = data
		},
		/**
		 * 设置批量操作模式下被选中的文件列表
