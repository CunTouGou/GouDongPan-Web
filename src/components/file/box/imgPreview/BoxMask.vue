...
export default {
	...
	watch: {
		...
		visible(val) {
			if (val) {
				...
				this.isShowMinImgList =
					localStorage.getItem('goudong_file_img_preview_show_min') !== 'false'
				this.$nextTick(() => {
		...
		activeIndex(newValue) {
			...
			this.$nextTick(() => {
				if (this.$refs.imgLarge[newValue].style.zoom) {
					this.imgZoom = Math.round(Number(
      ...
      this.inputActiveIndex = this.activeIndex + 1;
		},
		// 监听 是否展示缩略图列表，将变化值保存在 localStorage 中
		isShowMinImgList(val) {
			localStorage.setItem('goudong_file_img_preview_show_min', val)
		}
	},
	methods: {
		/**
		...
		handleCalculateImgZoom(currentIndex) {
			...
			this.imgZoom = Math.round(Number(zoomRate.toFixed(2)) * 100) //  缩放百分比
